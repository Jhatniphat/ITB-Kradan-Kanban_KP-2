  ...
  if (openOrClose) {
    ...
    if (overStatus.length > 0) {
      showToast(
        {
          status: "error",
          msg: `Has Over Limit Status : ${overStatus.join(",")}`,
        },
        5000
      );
    }
...
<template>
  ...
  <div class="opacity">
    ...
    <Modal :show-modal="showEditLimit">
      <div
        class="itbkk-modal-task flex flex-col gap-3 p-5 text-black bg-slate-50 rounded-lg w-3/4 m-auto"
      >
        <h1>Limit Status</h1>
        <hr />
        ...
        <div class="flex flex-row-reverse gap-4 mt-5">
          <button
            ...
            @click="EditLimitModal(false)"
          >
            Confirm
          </button>
          <button
            class="btn btn-outline btn-error"
            @click="showEditLimit = false"
          >
            Close
          </button>
        </div>
