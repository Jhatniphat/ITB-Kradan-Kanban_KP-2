...
<template>
  ...
  <div class="flex flex-col p-5 text-black bg-slate-50 rounded-lg w-full">
    ...
    <label class="form-control w-full">
      ...
      <input v-model="taskData.title" type="text" placeholder="Type here"
    ...
    <div class="flex flex-row gap-3">
      ...
      <label class="form-control basis-3/4">
        ...
        <textarea v-model="taskData.description" class="textarea textarea-bordered h-72" placeholder="Bio"></textarea>
        <div class="label">
          <!-- ? Error Text -->
          <span v-if="Errortext.description !== ''" class="label-text-alt text-error"> {{ Errortext.description
            }}</span>
        </div>
      </label>
      ...
      <div class="basis-1/4">
        ...
        <label class="form-control ">
          ...
          <textarea v-model="taskData.assignees" class="textarea textarea-bordered h-24" placeholder="Bio"></textarea>
          <div class="label">
            <!-- ? Error Text -->
            <span v-if="Errortext.assignees !== ''" class="label-text-alt text-error"> {{ Errortext.assignees
              }}</span>
          </div>
        </label>
        <!-- * status -->
    ...
    <div class="flex flex-row-reverse gap-4 mt-5">
      <button class="btn btn-outline  btn-error basis-1/6" @click="sendCloseModal()">Cancel</button>
      <button class="btn btn-outline btn-success basis-1/6" :disabled="!canSave" @click="fetchData()">
        {{ loading ? '' : 'Save' }}
